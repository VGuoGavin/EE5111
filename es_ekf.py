...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    s_var = np.eye(3) * sensor_var

    Kk = p_cov_check @ h_jac.T @ np.linalg.inv(h_jac @ p_cov_check @ h_jac.T + s_var)
    # 3.2 Compute error state
    delta_x_k = np.dot(Kk,(y_k - p_check))
    #delta_x_k = Kk @ (y_k - p_check).T
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_k = p_est[k-1] + v_est[k-1] * delta_t + 0.5 * (delta_t**2) * (f_ext + g)
    v_k = v_est[k-1] + delta_t * (f_ext + g)

    theta = imu_w.data[k - 1] * delta_t
    
    q_k = Quaternion(axis_angle=theta).quat_mult_right(q_est[k - 1])

    # 1.1 Linearize the motion model and compute Jacobians
    F_k_1 = np.eye(9)
    F_k_1[0:3, 3:6] = np.eye(3) * delta_t
    F_k_1[3:6, 3:6] = np.eye(3)
    # F_k_1[3:6, 6:9] = -(c_ns @ skew_symmetric(imu_f.data[k-1].reshape((3,1)))) * delta_t
    F_k_1[3:6, 6:9] = -skew_symmetric(c_ns @ imu_f.data[k - 1]) * delta_t
    ...
    p_cov_check = F_k_1 @ p_cov_check @ F_k_1.T + L_k_1 @ Q_km @ L_k_1.T

    # 3. Check availability of GNSS and LIDAR measurements
    if gnss_i < gnss.t.shape[0] and gnss.t[gnss_i] <= imu_f.t[k]:
       #if abs(gnss.t[gnss_i] - imu_f.t[k]) < 1E-6:
    ...
    if lidar_i < lidar.t.shape[0] and lidar.t[lidar_i] <= imu_f.t[k]:
        #if abs(lidar.t[lidar_i] - imu_f.t[k]) < 1E-6:
